  ...
  <div data-scrollbar="true" data-height="100%">
    ...
    <ul class="nav">
        ...
        <li class="has-sub <?php if ($current[0] == "customers" || $current[0] == "advice" || $current[0] == "installations")
          ...
          <ul class="sub-menu">
            <?php if (!empty($_SESSION['permits'][CLIENTS]['v'])) { ?>
              <li class="<?php if ($current[0] == "customers" && (empty($current[1]) || $current[1] == "add" || $current[1] == "view_client" || $current[1] == "customer_location"))
                echo "active"; ?>">
            ...
            <?php if (!empty($_SESSION['permits'][CLIENTS]['v'])) { ?>
              <li class="<?php if ($current[0] == "customers" && $current[1] == "resumen")
                echo "active"; ?>"><a href="<?= base_url() ?>/customers/resumen">Clientes Deuda</a></li>
            <?php } ?>
            <?php if (!empty($_SESSION['permits'][EMAIL]['v'])) { ?>
              <li class="<?php if ($current[0] == "advice")
                echo "active"; ?>"><a href="<?= base_url() ?>/advice">Correos</a>
              </li>
            <?php } ?>
      ...
      <?php if (!empty($_SESSION['permits'][RED]['v'])) { ?>
        <li class="has-sub <?php if (in_array($current[0], ["routerOS", "apclientes", "apemisor", "apreceptor", "cajaNap", "monitoreo"]))
          echo "active"; ?>">
          <a href="javascript:;">
            <b class="caret"></b>
          ...
          <ul class="sub-menu">
            <li class="<?php if ($current[0] == "routerOS")
              echo "active"; ?>">
              <a href="<?= base_url() ?>/routerOS">RouterOS</a>
            </li>
            ...
            <li class="<?php if ($current[0] == "cajaNap" && empty($current[1]))
              echo "active"; ?>">
              <a href="<?= base_url() ?>/cajaNap">Cajas Nap</a>
            </li>
            <li class="<?php if ($current[0] == "monitoreo")
              echo "active"; ?>">
              <a href="<?= base_url() ?>/monitoreo">Monitoreos</a>
            </li>
            <li class="<?php if ($current[0] == "cajaNap" && $current[1] == "view_map")
              echo "active"; ?>">
              <a href="<?= base_url() ?>/cajaNap/view_map">Mapa de Caja Nap</a>
            </li>
          </ul>
        </li>
      <?php } ?>
          ...
          <ul class="sub-menu">
            ...
            <?php if (!empty($_SESSION['permits'][CATEGORIES]['v'])) { ?>
              <li class="<?php if ($current[0] == "categories")
                echo "active"; ?>"><a href="<?= base_url() ?>/categories">Categorias</a></li>
            <?php } ?>
            <?php if (!empty($_SESSION['permits'][SUPPLIERS]['v'])) { ?>
              <li class="<?php if ($current[0] == "providers")
                echo "active"; ?>"><a href="<?= base_url() ?>/providers">Proveedores</a></li>
            <?php } ?>
            <?php if (!empty($_SESSION['permits'][PRODUCTS]['v'])) { ?>
              <li class="<?php if ($current[0] == "kardex")
                echo "active"; ?>"><a href="<?= base_url() ?>/kardex">Kardex</a>
              </li>
            <?php } ?>
          </ul>
        </li>
          ...
          <ul class="sub-menu">
            ...
            <?php if (!empty($_SESSION['permits'][TICKETS]['v'])) { ?>
              <li class="<?php if ($current[0] == "tickets" && $current[1] == "expired")
                echo "active"; ?>"><a href="<?= base_url() ?>/tickets/expired">Vencidos</a></li>
            <?php } ?>
            <?php if (!empty($_SESSION['permits'][TICKETS]['v'])) { ?>
            ...
            <?php if (!empty($_SESSION['permits'][BILLS]['v'])) { ?>
              <?php if ($_SESSION['userData']['profileid'] == ADMINISTRATOR) { ?>
                <li class="<?php if ($current[0] == "bills" && empty($current[1]))
                  echo "active"; ?>"><a href="<?= base_url() ?>/bills">Lista de facturas</a></li>
              <?php } ?>
            <?php } ?>
            <?php if (!empty($_SESSION['permits'][BILLS]['v'])) { ?>
              <li class="<?php if ($current[0] == "bills" && $current[1] == "pendings")
                echo "active"; ?>"><a href="<?= base_url() ?>/bills/pendings">Facturas pendientes</a></li>
            <?php } ?>
            <?php if (!empty($_SESSION['permits'][PAYMENTS]['r'])) { ?>
              <li class="<?php if ($current[0] == "payments" && $current[1] == "add_payment")
                echo "active"; ?>"><a href="<?= base_url() ?>/payments/agregarPagos">Registrar pago</a></li>
            <?php } ?>
            <?php if (!empty($_SESSION['permits'][PAYMENTS]['v'])) { ?>
              <?php if ($_SESSION['userData']['profileid'] != TECHNICAL && $_SESSION['userData']['profileid'] != CHARGES) { ?>
                <li class="<?php if ($current[0] == "payments" && empty($current[1]))
                  echo "active"; ?>"><a href="<?= base_url() ?>/payments">Cobranzas realizadas</a></li>
              <?php } ?>
            <?php } ?>
            <?php if (!empty($_SESSION['permits'][PAYMENTS]['v'])) { ?>
              <?php if ($_SESSION['userData']['profileid'] == ADMINISTRATOR) { ?>
                <li class="<?php if ($current[0] == "payments" && $current[1] == "statistics")
                  echo "active"; ?>"><a href="<?= base_url() ?>/payments/statistics">Estadisticas</a></li>
              <?php } ?>
            <?php } ?>
            <?php if (!empty($_SESSION['permits'][PAYMENTS]['v'])) { ?>
              <?php if ($_SESSION['userData']['profileid'] == ADMINISTRATOR) { ?>

<?php
	class PaymentsModel extends Mysql{
		...
		public function list_records(string $start,string $end,string $type,string $user,string $state){
				if (isset($start) && isset($end) && isset($type) && isset($user) && isset($state) && !empty($start) && !empty($end) && !empty($type) && !empty($user) && !empty($state)){
						...
						WHERE DATE(p.payment_date) >= '$start' AND DATE(p.payment_date) <= '$end' AND p.paytypeid = '$type' AND p.userid = $user AND p.state = $state ORDER BY p.id DESC";
						...
						JOIN vouchers v ON b.voucherid = v.id
						JOIN voucher_series vs ON b.serieid = vs.id
						WHERE DATE(p.payment_date) >= '$start' AND DATE(p.payment_date) <= '$end' ORDER BY p.id DESC";
						...
						JOIN vouchers v ON b.voucherid = v.id
						JOIN voucher_series vs ON b.serieid = vs.id
						WHERE DATE(p.payment_date) >= '$start' AND DATE(p.payment_date) <= '$end' AND p.paytypeid = '$type' ORDER BY p.id DESC";
						...
						JOIN vouchers v ON b.voucherid = v.id
						JOIN voucher_series vs ON b.serieid = vs.id
						WHERE DATE(p.payment_date) >= '$start' AND DATE(p.payment_date) <= '$end' AND p.userid = $user ORDER BY p.id DESC";
						...
						JOIN vouchers v ON b.voucherid = v.id
						JOIN voucher_series vs ON b.serieid = vs.id
						WHERE DATE(p.payment_date) >= '$start' AND DATE(p.payment_date) <= '$end' AND p.state = $state ORDER BY p.id DESC";
					...
					JOIN vouchers v ON b.voucherid = v.id
					JOIN voucher_series vs ON b.serieid = vs.id
					WHERE DATE(p.payment_date) >= '$start' AND DATE(p.payment_date) <= '$end' AND p.paytypeid = '$type' AND p.userid = $user ORDER BY p.id DESC";
					...
					JOIN vouchers v ON b.voucherid = v.id
					JOIN voucher_series vs ON b.serieid = vs.id
					WHERE DATE(p.payment_date) >= '$start' AND DATE(p.payment_date) <= '$end' AND p.paytypeid = '$type' AND p.state = $state ORDER BY p.id DESC";
						...
						JOIN vouchers v ON b.voucherid = v.id
						JOIN voucher_series vs ON b.serieid = vs.id
						WHERE DATE(p.payment_date) >= '$start' AND DATE(p.payment_date) <= '$end' AND p.userid = $user AND p.state = $state ORDER BY p.id DESC";
				}else{
      ...
      $this->intClient = $client;
      $sql = "SELECT b.id,b.clientid,b.internal_code,b.correlative,b.date_issue,b.expiration_date,b.billed_month,b.discount,b.subtotal,b.total,b.type,b.sales_method,b.remaining_amount,b.amount_paid,b.state,vs.serie,v.voucher,b.observation, b.compromise_date
      FROM bills b
      JOIN vouchers v ON b.voucherid = v.id
      JOIN voucher_series vs ON b.serieid = vs.id
    ...
    public function modify_compromise(string $id, $compromise_date = null){
      $this->intBill = $id;

			$answer = "";
			$query = "UPDATE bills SET compromise_date = ? WHERE id = $this->intBill";
			$data = array($compromise_date);
			$update = $this->update($query,$data);
      ...
      return $answer;
		...
		public function massive_bills($bills){
			...
			$sql_bill = "SELECT b.id,b.clientid,b.voucherid,b.serieid,c.names,c.surnames,dt.document AS type_doc,c.document,c.mobile,c.mobile_optional,c.address,c.email,v.voucher,vs.serie,b.internal_code,b.observation,b.date_issue,b.billed_month,b.correlative,b.expiration_date,b.subtotal,b.discount,b.total,b.amount_paid,b.remaining_amount,b.type,b.sales_method,b.state, b.compromise_date

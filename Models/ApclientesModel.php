...
class ApClientesModel extends Mysql {
  ...
  public function list_users_record(string $id) {
    return $this->createQueryBuilder()
      ...
      ->where("ap.id = '{$id}'")
      ->select("c.id,c.internal_code,c.clientid,c.payday,c.create_invoice,c.days_grace,c.discount,c.discount_price,c.months_discount,c.contract_date,c.suspension_date,c.finish_date,c.state,CONCAT_WS(' ', cl.names, cl.surnames) AS client,cl.document,d.document AS name_doc,cl.latitud,cl.longitud,cl.email,cl.mobile,cl.mobile_optional,cl.address,cl.reference, cl.ap_cliente_id")

<?php
	class InstallationsModel extends Mysql{
		...
		public function list_records(int $state){
			if($state == 0){
				$sql = "SELECT i.id,i.clientid,i.attention_date,i.opening_date, i.closing_date,i.cost,i.technical,i.detail,i.state,CONCAT_WS(' ', c.names, c.surnames) AS client,c.mobile,c.mobile_optional,c.zona,c.address,c.reference,c.latitud,c.longitud,c.document,d.document AS name_doc,u.names AS user,i.clientid
	      FROM facility i
		...
		public function select_record(int $id){
			$this->intId = $id;
			$sql = "SELECT f.id,f.clientid,f.technical,f.attention_date,f.opening_date,f.closing_date,f.cost,f.detail,f.state,c.names,c.surnames,c.document,c.mobile,c.mobile_optional,c.zona,c.email,c.address,c.reference, ap.nombre ap_cliente_text
			FROM facility f
      ...
      if($insert){
		...
		public function view_installation(int $facility){
			$request = array();
			$sql_facility = "SELECT f.id,CONCAT_WS(' ', c.names, c.surnames) AS client,dt.document AS type_doc,c.document,c.mobile,c.mobile_optional,c.zona,c.address,c.reference,c.email,f.clientid,f.attention_date,f.registration_date,f.technical,f.cost,f.detail,f.state,CONCAT_WS(' ', u.names, u.surnames) AS user,u.image AS user_image
			FROM facility f
			JOIN clients c ON f.clientid = c.id

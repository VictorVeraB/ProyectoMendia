...
class CustomersModel extends Mysql
{
  ...
  private $intService;
  /* VARIABLES CLIENTES */
  private $intClient, $strNames, $strSurnames, $intType, $strDocument, $strMobile, $strMobileOp, $strzona, $strEmail, $strAddess, $strReference, $strLatitud, $strLongitud;
  /* VARIABLS USUARIOS */
  private $intUser;
  ...
  {
    // numero de duda
    $orderDeuda = isset($filters['orderDeuda']) ? $filters['orderDeuda'] : null;
    ...
    $queryBuilder = $this->createQueryBuilder()
      ->select(implode(",", [
        ...
        "cl.mobile, cl.mobile_optional, cl.address, cl.reference, cl.ap_cliente_id"
      ]))
      ->from("contracts c")
  ...
  {
    $sql = "SELECT c.id,c.internal_code,c.clientid,c.payday,c.create_invoice,c.days_grace,c.discount,c.discount_price,c.months_discount,c.contract_date,c.suspension_date,c.finish_date,c.state,cl.names,cl.surnames,cl.document,d.document AS name_doc,cl.latitud,cl.longitud,cl.email,cl.mobile,cl.mobile_optional,cl.zona,cl.address,cl.reference FROM contracts c JOIN clients cl ON c.clientid = cl.id JOIN document_type d ON cl.documentid = d.id WHERE c.state != 0 ORDER BY c.id ASC";
  ...
  public function modify(int $id, int $payday, int $invoice, int $daygrace, int $discount, string $price, string $month, int $state)
  {
    ...
    $this->intState = $state;
    $answer = "";
    $query = "UPDATE contracts SET payday=?,create_invoice=?,days_grace=?,discount=?,discount_price=?,months_discount=?,remaining_discount=? WHERE id = $this->intId";
    $data = array($this->intPayDay, $this->intInvoice, $this->intDayGrace, $this->intDiscount, $this->strPrice, $this->intMonth, $this->intMonth);
  ...
  public function import_client(string $names, string $surnames, int $type, string $document, string $mobile, string $mobileOp, string $zona, string $email, string $address, string $reference, string $latitud, string $longitud)
  {
    $this->strNames = $names;
    ...
    $answer = "";
    $query = "INSERT INTO clients(names,surnames,documentid,document,mobile,mobile_optional,zona,email,address,reference,latitud,longitud) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
    $data = array($this->strNames, $this->strSurnames, $this->intType, $this->strDocument, $this->strMobile, $this->strMobileOp, $this->strzona, $this->strEmail, $this->strAddess, $this->strReference, $this->strLatitud, $this->strLongitud);
    $insert = $this->insert($query, $data);
    if ($insert) {
      $answer = 'success';
    } else {
  ...
  public function create_client(string $names, string $surnames, int $type_document, string $document, string $mobile, string $mobileOp, string $zona, string $email, string $address, string $reference, string $apCliente)
  {
    $this->strNames = $names;
    $this->strSurnames = $surnames;
    ...
    $this->strReference = $reference;
    $answer = "";
    $query = "INSERT INTO clients(names,surnames,documentid,document,mobile,mobile_optional,zona,email,address,reference, ap_cliente_id) VALUES(?,?,?,?,?,?,?,?,?,?,?)";
    $data = array($this->strNames, $this->strSurnames, $this->intType, $this->strDocument, $this->strMobile, $this->strMobileOp, $this->strzona, $this->strEmail, $this->strAddess, $this->strReference, $apCliente);
    $insert = $this->insert($query, $data);
    if ($insert) {
      $answer = 'success';
      if (isset($mobileOp)) {
        $this->block_ip($insert, $mobileOp);
      }
    } else {
      $answer = 'error';
    }
  ...
  public function saveClient($data = [])
  {
    try {
      ...
      $mobileOp = $data['mobile_optional'];
      if ($clientId) {
        if (isset($mobileOp))
          $this->block_ip($clientId, $mobileOp);
        return "success";
      } else {
        return "error";
      }
  ...
  public function editClient(int $id, $data = [])
  {
    ...
    $isUpdated = $this->createQueryBuilder()
      ...
      ->set($data)
      ->execute();
    $mobile_optional = $data['mobile_optional'] ? $data['mobile_optional'] : $client['mobile_optional'];
    if ($isUpdated && $mobile_optional)
      $this->block_ip($id, $mobile_optional);
    return $isUpdated ? "success" : "error";
  }

  public function modify_client(int $id, string $names, string $surnames, int $type_document, string $document, string $mobile, string $mobileOp, string $zona, string $email, string $address, string $reference, string $latitud, string $longitud, string $apCliente)
  {
    $this->intClient = $id;
    $this->strNames = $names;
    $this->strSurnames = $surnames;
    ...
    $request = $this->select_all($sql);
    if (empty($request)) {
      $query = "UPDATE clients SET names=?,surnames=?,documentid=?,document=?,mobile=?,mobile_optional=?,zona=?,email=?,address=?,reference=?,latitud=?,longitud=?, ap_cliente_id=? WHERE id = $this->intClient";
      $data = array(
        $this->strNames,
        $this->strSurnames,
        $this->intType,
  ...
  public function block_ip(int $clientId, string $ipId)
  {
    $query = "UPDATE ips i SET i.estado = ? WHERE i.estado = 'OCUPADO'";
    $query .= "AND NOT EXISTS(SELECT * FROM clients c ";
    $query .= "WHERE c.mobile_optional = i.id)";
    $this->update($query, ["LIBRE"]);
    if (isset($ipId)) {
      return $this->update("UPDATE ips SET estado = ? WHERE id = '{$ipId}'", ["OCUPADO"]);
    } else {
      return false;
    }
  }

  public function free_ip(int $contractId)
  {
    $client = $this->createQueryBuilder()
      ->from('contracts as con')
      ->select("cli.id, cli.mobile_optional")
      ->innerJoin("clients as cli", "cli.id = con.clientid")
      ->where("con.id = {$contractId}")
      ->getOne();
    if (!isset($client))
      return false;
    $clientId = $client['id'];
    $ipId = $client['mobile_optional'];
    $this->update("UPDATE ips SET estado = ? where id = ?", ['LIBRE', $ipId]);
    $this->update("UPDATE clients SET mobile_optional = null WHERE id = ?", [$clientId]);
    return true;
  }
  ...
  {
    $this->intClient = $client;
    $sql = "SELECT c.id,c.names,c.surnames, CONCAT(c.names, CONCAT(' ', c.surnames)) client,
        c.documentid,c.document,c.mobile,c.mobile_optional,
        c.zona,c.email,c.address,c.reference,c.latitud,c.longitud, c.state,d.document AS name_doc,
        c.nap_cliente_id, c.opcion, c.ppoe_usuario, c.ppoe_password
        FROM clients c JOIN document_type d ON c.documentid = d.id WHERE c.id = $this->intClient";
  ...
  public function bill_voucher(int $id)
  {
    ...
    $sql_bill = "SELECT b.id,b.clientid,b.voucherid,b.serieid,c.names,c.surnames,
        ...
        b.remaining_amount,b.type,b.sales_method,b.state, b.compromise_date 
  ...
  public function remaining_amount_data(int $bill)
  {
    $this->intBill = $bill;
    $sql = "SELECT remaining_amount AS total, subtotal, amount_paid, discount FROM bills WHERE id = $this->intBill";
    $answer = $this->select($sql);
    return [
      "total" => $answer["total"],
      "subtotal" => $answer["subtotal"],
  ...
  public function modify_amounts(int $bill, string $subscriber, string $remaining, int $state, int $discount, string $compromise_date = null)
  {
    $this->intBill = $bill;
    ...
    $answer = "";
    if ($this->intState == 0) {
      $query = "UPDATE bills SET discount = ?, total = subtotal - ?, compromise_date = ?, amount_paid = amount_paid + ?,remaining_amount = ? WHERE id = $this->intBill";
      $data = array($discount, $discount, $compromise_date, $this->strSubscriber, $this->strRemaining);
    } else {
      $query = "UPDATE bills SET discount = ?, total = subtotal - ?, compromise_date = ?, amount_paid = amount_paid + ?,remaining_amount = ?,state = ? WHERE id = $this->intBill";
      $data = array($discount, $discount, $compromise_date, $this->strSubscriber, $this->strRemaining, $this->intState);
    }
    $update = $this->update($query, $data);
    if ($update) {
      $answer = 'success';
  ...
  {
    $this->intPayment = $payment;
    $sql = "SELECT p.id,p.billid,p.userid,p.clientid,CONCAT_WS(' ', c.names, c.surnames) AS client,p.internal_code,p.paytypeid,p.payment_date,p.comment,p.amount_paid,p.amount_total, b.subtotal, b.discount, p.remaining_credit,vs.serie,v.voucher,b.correlative
      FROM payments p

...
function ip_option(show = false, required = false) {
  ...
  const currentIp = document.getElementById("mobileOpCurrent");
  const ip = document.getElementById("mobileOp");
  if (show) {
    content.prop("style", "display: flex");
    ...
    ip.setAttribute("value", ip.value || currentIp.value);
  } else {
    content.prop("style", "display: none");
...
function corte_firewall_option(show = false, required = false) {
  const content = $("#content-is_firewall");
  if (show) {
    content.prop("style", "display: flex");
  } else {
    content.prop("style", "display: none");
...
function changeOpcion() {
  ...
  if (opcion.val() === "NINGUNO") {
    ...
    corte_firewall_option(true, false);
...
function validateMikrotikDefault(component) {
  const mikrotik = $("#mikrotik_default").val();
  if (!mikrotik) return;
  component.val(mikrotik);
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  changeListTypes();
  onCheckIp();
  generateMikrotikSelect().then(validateMikrotikDefault);
});

...
function listeners() {
  ...
  document.getElementById("mobileOp").addEventListener("keyup", search_ip);
}
  ...
  function () {
          ...
          request.onreadystatechange = function () {
            if (request.readyState == 4 && request.status == 200) {
              ...
              if (objData.status == "success") {
                $("#mobileOpCurrent").val($("#mobileOp").val());
    ...
    if (document.querySelector("#transactions_internet")) {
      ...
      transactions_internet.onsubmit = function (e) {
        ...
        if ($("#transactions_internet").parsley().isValid()) {
          ...
          formData.set("price", formatDecimal(formData.get("price")));
          request.open("POST", ajaxUrl, true);
          request.send(formData);
          request.onreadystatechange = function () {
            if (request.readyState == 4 && request.status == 200) {
    ...
    if (document.querySelector("#transactions_free")) {
      ...
      transactions_free.onsubmit = function (e) {
        ...
        formData.set("discount", formatDecimal(formData.get("discount")));
        formData.set("subtotal", formatDecimal(formData.get("subtotal")));
        formData.set("total", formatDecimal(formData.get("total")));
        const costos = formData.getAll("costo[]");
        const totales = formData.getAll("totales[]");
        formData.delete("costo[]");
        formData.delete("totales[]");
        costos.forEach((value) =>
          formData.append("costo[]", formatDecimal(value))
        );
        totales.forEach((value) =>
          formData.append("totales[]", formatDecimal(value))
        );
        request.open("POST", ajaxUrl, true);
        request.send(formData);
        request.onreadystatechange = function () {
                  ...
                  if (objData.remaining_amount == 0) {
                    ...
                    if (objData.type == 1) {
                      if (objData.amount_paid == 0) {
                        ...
                        var msg = `Hola, se registro ${
                          objData.symbol + formatMoney(objData.amount_paid)
                        } al ${objData.voucher.toLowerCase()} número ${
                          objData.serie
                        }, quedando un saldo pendiente de ${
                          objData.symbol + formatMoney(objData.remaining_amount)
                        } del cliente ${
                          objData.client
                        }. Puede revisarlo en el siguiente enlace: ${url}. Muchas gracias por su pago, Atte. ${
                          objData.business_name
    ...
    if (document.querySelector("#transactions_payments")) {
      ...
      transactions_payments.onsubmit = function (e) {
        ...
        if ($("#transactions_payments").parsley().isValid()) {
          ...
          formData.set(
            "remaining_amount",
            formatDecimal($("#remaining_amount").val())
          );
          formData.set(
            "total_discount",
            formatDecimal($("#total_discount").val())
          );
                    ...
                    if (objData.remaining_amount == 0) {
                      if (objData.type == 1) {
                        ...
                        var msg = `Estimado cliente *${
                          ...
                          month
                        ).toUpperCase()},* Puede revisarlo en el siguiente enlace: ${url}. Muchas gracias por su pago, Atte. *${
                          objData.business_name
                        }*`;
                      }
                    } else {
                      if (objData.type == 1) {
                        if (objData.amount_paid == 0) {
                          ...
                          var msg = `Hola, se registro ${
                            ...
                            objData.symbol +
                            formatMoney(objData.remaining_amount)
                          } del cliente ${
                            objData.client
                          }. Puede revisarlo en el siguiente enlace: ${url}. Muchas gracias por su pago, Atte. ${
                            objData.business_name
...
window.addEventListener(
  ...
  function () {
    ...
    $("#freeissue,#freeexpiration,#servissue,#servexpiration")
      .datetimepicker({
        locale: "es",
        format: "L",
      })
      .on("dp.change", function (e) {
        if (e.currentTarget?.id === "servissue") onChangeIssue();
      });
      ...
      if (objData.status == "success") {
        if (objData.data.service) {
          if (objData.data.invoice.status == 4) {
            ...
            document.querySelector("#discountserv").value = formatMoney(
            ...
            billedMonthUpdate(parseDate(objData.data.invoice.billed_month));
          }
        } else {
  ...
  request.onreadystatechange = function () {
    if (request.readyState == 4 && request.status == 200) {
      ...
      if (objData.status == "success") {
        ...
        $("#idpayment").val("");
        $("#idbillpayment").val(objData.data.encrypt_bill);
        $("#idbillclient").val(objData.data.encrypt_client);
        $("#total_pay").val(formatMoney(objData.data.remaining_amount));
        $("#subtotal").val(formatMoney(objData.data.subtotal));
        $("#amount_paid").val(formatMoney(objData.data.amount_paid));
        $("#remaining_amount").val(formatMoney(objData.data.remaining_amount));
        $("#discount").val(formatMoney(objData.data.discount));
        $("#total_discount").val(formatMoney(objData.data.discount));
        $("#date_time").val(moment().format("DD/MM/YYYY H:mm"));
        document.querySelector("#total_pay").readOnly = false;
        $("#date_compromiso")
...
function addItem(option) {
  if (option == "libre") {
    items = `
        ...
        <td><input type="text" class="form-control costo" placeholder="100" name="costo[]" value="${formatMoney(
          0
        )}"></td>
        <td><input type="number" class="form-control unidad" placeholder="1" min="1" name="unidad[]" value="1"></td>
        <td><input type="text" class="form-control total" placeholder="100"  name="totales[]" value="${formatMoney(
          0
        )}"></td>
        <td class="text-center"><button type="button" class="deletefile btn btn-white"><i class="far fa-trash-alt"></i></button></td>
        </tr>
    ...
    items = `
        <tr class="item-row2">
          ...
          <td class="text-center">
            <button type="button" 
              class="deletefile2 btn btn-white"
            >
...
$("#table-free,#discountfree").on("change", function () {
  calculateTotals(
    ".item-row",
    ".costo",
    ".unidad",
...
$("#table-plans,#discountserv").on("change", function () {
  calculateTotals(
    ".item-row2",
    ".costos",
    ".unidads",
...
function calculateTotals(
  ...
  $(itemRow).each(function (i) {
    const row = $(this);
    const valPrice = formatDecimal(row.find(price).val());
    const valQtycl = formatDecimal(row.find(qtycl).val());
    var total = valPrice * valQtycl;
    row.find(totalcl).val(formatMoney(total));
    row.find(price).val(formatMoney(valPrice));
  });
  //calcular el subtotal
  var subtotal = 0;
  $(totalcl).each(function (i) {
    var total = formatDecimal($(this).val());
    if (!isNaN(total)) subtotal += Number(total);
  });
  subtotal = formatDecimal(subtotal);
  $(subTotal).val(formatMoney(subtotal));
  $(textsub).text(currency_symbol + formatMoney(subtotal));
  //calcular el total
  var grandTotal =
    Number(formatDecimal($(subTotal).val())) -
    Number(formatDecimal($(discount).val()));
  grandTotal = formatMoney(grandTotal);
  const valDiscount = formatMoney($(discount).val());
  $(totalNeto).val(grandTotal);
  $(textto).text(currency_symbol + grandTotal);
  $(textdis).text(currency_symbol + valDiscount);
  $(discount).val(valDiscount);
}
    ...
    if (request.readyState == 4 && request.status == 200) {
      ...
      if (objData.status == "success") {
        ...
        document.querySelector("#view-sub").innerHTML =
          currency_symbol + formatMoney(objData.data.bill.subtotal);
        document.querySelector("#view-dis").innerHTML =
          currency_symbol + formatMoney(objData.data.bill.discount);
        document.querySelector("#view-total").innerHTML =
          currency_symbol + formatMoney(objData.data.bill.total);
          ...
          objData.data.payments.forEach((payment) => {
            template_trans += `
                          <tr>
                              ...
                              <td class="text-center">${currency_symbol}${formatMoney(
              payment.amount_paid
            )}</td>
  ...
  request.onreadystatechange = function () {
    if (request.readyState == 4 && request.status == 200) {
      ...
      if (objData.status == "success") {
        ...
        billedMonthUpdate(objData.data.bill.billed_month);
        ...
        document.querySelector("#subtotalserv").value = formatMoney(
        ...
        document.querySelector("#text-sub").innerHTML = formatMoney(
          objData.data.bill.subtotal
        );
        document.querySelector("#text-dis").innerHTML = formatMoney(
          objData.data.bill.discount
        );
        document.querySelector("#text-total").innerHTML = formatMoney(
            ...
            if (detail.type == 1) {
              template += `
                            <tr class="item-row2">
                              ...
                              <td class="text-center">
                                <button type="button" class="deletefile2 btn btn-white">
                                  <i class="far fa-trash-alt"></i>
                                </button>
                              </td>
                            </tr>
      ...
      if (objData.status) {
        ...
        if (objData.data.bill.remaining_amount == 0) {
          if (objData.data.bill.type == 1) {
            var msg = `Hola, se registro ${
              objData.data.business.symbol + objData.data.bill.amount_paid
            } al ${objData.data.bill.voucher.toLowerCase()} número ${
              objData.data.bill.serie
            } del cliente ${
              objData.data.bill.client
            }. Puede revisarlo en el siguiente enlace: ${url}. Muchas gracias por su pago, Atte. ${
              objData.data.business.business_name
            }`;
          } else {
            ...
            var msg = `HOLA, *${
              objData.data.bill.client
            }*\n\n Se registro su pago de *${
              objData.data.business.symbol +
              formatMoney(objData.data.bill.amount_paid)
            }* al ${objData.data.bill.voucher.toLowerCase()} número *${
              objData.data.bill.serie
            } de ${month_letters(
              month
            ).toUpperCase()}*.\n  Puede revisarlo en el siguiente enlace: ${url}\n Muchas gracias por su pago.\n\n *${
          ...
          if (objData.data.bill.type == 1) {
            if (objData.data.bill.amount_paid == 0) {
              var msg = `Hola, esta pendiente el pago de ${
                objData.data.business.symbol +
                objData.data.bill.remaining_amount
              } del ${objData.data.bill.voucher.toLowerCase()} número ${
                objData.data.bill.serie
              } del cliente ${
                objData.data.bill.client
              }. Puede revisarlo en el siguiente enlace: ${url}. Muchas gracias por su pago, Atte. ${
              ...
              var msg = `Hola, se registro ${
                ...
                objData.data.business.symbol +
                formatMoney(objData.data.bill.remaining_amount)
              } del cliente ${
                objData.data.bill.client
              }. Puede revisarlo en el siguiente enlace: ${url}. Muchas gracias por su pago, Atte. ${
                objData.data.business.business_name
...
function update_payment(idpayment) {
  ...
  axios
    .get(`${base_url}/customers/select_payment/${idpayment}`)
    .then(({ data }) => {
      if (data.status !== "success") throw new Error(data.message);
      const info = data.data;
      var formPayments = document.querySelector("#transactions_payments");
      formPayments.reset();
      document.querySelector(
        "#text-payment"
      ).innerHTML = `Agregar Pago ${info.voucher} N° ${info.invoice}`;
      document.querySelector("#client_name").innerHTML = info.client;
      document.querySelector("#text-alert").innerHTML =
        '<small class="text-success text-uppercase">El total pagado no es editable</small>';
      document.querySelector("#idpayment").value = info.encrypt_id;
      document.querySelector("#idbillpayment").value = info.encrypt_bill;
      document.querySelector("#date_time").value = info.date;
      document.querySelector("#idbillclient").value = info.encrypt_client;
      document.querySelector("#total_pay").value = formatMoney(
        info.amount_paid
      );
      $("#remaining_amount").val(formatMoney(info.amount_paid));
      $("#subtotal").val(formatMoney(info.subtotal));
      $("#amount_paid").val(formatMoney(info.amount_paid));
      $("#total_discount").val(formatMoney(info.discount));
      $("#total_discount").prop("disabled", true);
      $("#total_pay").prop("disabled", true);
      document.querySelector("#comment").value = info.comment;
      document.querySelector("#listTypePay").value = info.paytypeid;
      $("#listTypePay").select2({ width: "100%" });
      $("#modal-payment").modal("show");
    })
...
function select_internet(idservices) {
  ...
  request.onreadystatechange = function () {
    if (request.readyState == 4 && request.status == 200) {
      var objData = JSON.parse(request.responseText);
      if (objData.status) {
        document.querySelector("#price_internet").value = formatMoney(
          objData.data.price
        );
...
function select_personalized(idservices) {
  ...
  request.onreadystatechange = function () {
    if (request.readyState == 4 && request.status == 200) {
      var objData = JSON.parse(request.responseText);
      if (objData.status == "success") {
        document.querySelector("#price_personalized").value = formatMoney(
          objData.data.price
        );
  ...
  request.onreadystatechange = function () {
    if (request.readyState == 4 && request.status == 200) {
      ...
      if (objData.status == "success") {
        ...
        if (galleryEl) galleryEl.innerHTML = htmlImage;
      }
    }
  };
}
...
function calcTotal() {
  ...
  return formatDecimal(subtotal - (amountPaid + discount));
}
...
function changePay(evt) {
  const amountDefault = formatDecimal($("#remaining_amount").val());
  const inputPay = $("#total_pay");
  const deuda = calcTotal();
  const current = parseFloat(formatDecimal(inputPay.val()));
  if (deuda < current) {
    alert_msg("warning", "El monto supera la deuda.");
    inputPay.val(formatMoney(amountDefault));
  } else if (current == 0) {
    alert_msg("error", "El monto no puede ser mayor/igual a cero");
    inputPay.val(formatMoney(amountDefault));
  } else {
  ...
  searchComponent("mobileOp", "ip", loadingIp)
    .then(
      ({
        ...
        const renderDefault = () => {
          const current = $("#mobileOpCurrent").val();
          if (!current) return;
          const itemDefault = renderItem(`IP [${current}]`);
          itemDefault.onclick = () => actionItem(input, current);
        };
        ...
        axios
          ...
          .then(({ data }) => {
            ...
            data.forEach((ip) => {
              ...
              item.onclick = () => actionItem(input, ip.id);
            });
          })
          .catch(() => renderNotFound(renderDefault))
          .finally(() => closeEvent());
      }
    )
    .catch((err) => alert_msg("warning", err.message));
...
function onChangeIssue() {
  const dateIssue = $("#servissue").val();
  billedMonthUpdate(parseDate(dateIssue));
}

function billedMonthUpdate(value) {
  const currentDate = moment(value).format("DD/MM/YYYY");
  $("#billed_month").val(currentDate);
}

function updateRouter(event) {
  event.preventDefault();
  const isValid = $("#update_router").parsley().isValid();
  if (!isValid) return;
  const clientId = $("#clientId").val();
  const form = new FormData(document.getElementById("update_router"));
  loading.style.display = "flex";
  axios
    .post(`${base_url}/customers/update_router/${clientId}`, form)
    .then(({ data }) => {

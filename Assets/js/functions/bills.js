                ...
                if (objData.modal) {
                  $("#modal-payment").on("hidden.bs.modal", function () {
                    ...
                    if (objData.remaining_amount == 0) {
                      if (objData.type == 1) {
                        var msg = `Estimado cliente, se registro ${
                      ...
                      if (objData.type == 1) {
                        if (objData.amount_paid == 0) {
                          var msg = `Estimado cliente, esta pendiente el pago de ${
                            objData.symbol +
...
window.addEventListener(
  ...
  function () {
    ...
    $("#total_payment").on("change", () => {
      const value = $("#total_payment").val();
      $("#total_payment").val(formatMoney(value));
    });
    $(
...
function checkDocument() {
  ...
  if (!id) return;
  const documentData = JSON.parse($("#documentData").val()) || [];
  ...
  if (option == "libre") {
    items = `
        ...
        <td class="text-center">
          <button type="button" class="deletefile btn btn-white">
    ...
    items = `
      <tr class="item-row2">
        <td>
          <input type="hidden" name="idproducto[]" value="0"><input type="hidden" name="tipo[]" value="1">
          <input class="form-control text-uppercase" placeholder="Descripción" name="descripcion[]" value=""></td>
        <td>
          <input type="text"
            class="form-control costos"
            placeholder="100"
          ...
          objData.data.payments.forEach((payment) => {
            template_trans += `
                <tr>
                  ...
                  <td class="text-center">${moment(payment.payment_date).format(
                    "DD/MM/YYYY H:mm"
                  )}</td>
                  <td class="text-center">
                  ${currency_symbol}${formatMoney(payment.amount_paid)}
                  </td>
          ...
          objData.data.detail.forEach((detail) => {
            template += `
              <tr>
                ...
                <td class="text-center">
                  ${currency_symbol}${formatMoney(detail.price)}
                </td>
                <td class="text-center">
                  ${detail.quantity}
                </td>
                <td class="text-center">
                  ${currency_symbol}${formatMoney(detail.total)}
                </td>
              </tr>
            `;
          });
  ...
  request.onreadystatechange = function () {
    if (request.readyState == 4 && request.status == 200) {
      ...
      if (objData.status == "success") {
        ...
        document.querySelector("#total_payment").value = formatMoney(
          objData.data.remaining_amount
        );
        $("#total_payment").trigger("touchspin.updatesettings", {
          max: formatDecimal(objData.data.remaining_amount),
        });
        $("#date_time").val(moment().format("DD/MM/YYYY H:mm"));
        list_runway();
  ...
  request.onreadystatechange = function () {
    if (request.readyState == 4 && request.status == 200) {
      ...
      if (objData.status == "success") {
        ...
        document.querySelector("#totalserv").value = formatMoney(

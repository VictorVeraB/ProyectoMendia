      ...
      transactions.onsubmit = function (e) {
        ...
        if ($("#transactions").parsley().isValid()) {
          ...
          if (providers == "") {
            alert_msg(
              "error",
              "No seleccionaste ningún proveedor, seleccione uno."
            );
            return false;
          }
          if (categories == "") {
            alert_msg(
              "error",
              "No seleccionaste ningúna categoria, seleccione uno."
            );
            return false;
          }
          ...
          formData.set(
            "purchase_price",
            formatDecimal(formData.get("purchase_price"))
          );
          formData.set("sale_price", formatDecimal(formData.get("sale_price")));
...
function list_providers() {
  if (document.querySelector("#listProviders")) {
    ...
    request.onreadystatechange = function () {
      if (request.readyState == 4 && request.status == 200) {
        document.querySelector("#listProviders").innerHTML =
          request.responseText;
  ...
  request.onreadystatechange = function () {
    if (request.readyState == 4 && request.status == 200) {
      ...
      if (objData.status == "success") {
        ...
        document.querySelector("#purchase_price").value = formatMoney(
          objData.data.purchase_price
        );
        document.querySelector("#sale_price").value = formatMoney(
          objData.data.sale_price
        );
        document.querySelector("#stock").value = objData.data.stock;
        document.querySelector("#stock_alert").value = objData.data.stock_alert;
        document.querySelector("#listCategories").value =
          objData.data.encrypt_category;
        $("#listCategories").select2({ width: "100%" });
        document.querySelector("#listUnits").value = objData.data.encrypt_unit;
        $("#listUnits").select2({ width: "100%" });
        document.querySelector("#listProviders").value =
          objData.data.encrypt_provider;
        $("#listProviders").select2({ width: "100%" });
        ...
        if (objData.data.image == "no_image.jpg") {
          document.querySelector("#img_product").src =
            base_url + "/Assets/images/default/no_image.jpg";
        } else {
          fetch(objData.data.url_image, { method: "HEAD" })
            .then((answer) => {
              if (answer.ok) {
                document.querySelector("#img_product").src =
                  objData.data.url_image;
              } else {
                console.log("not");
                document.querySelector("#img_product").src =
                  base_url + "/Assets/images/default/no_image.jpg";

...
document.addEventListener(
  ...
  function () {
    ...
    if (document.querySelector("#transactions_whatsapp")) {
      var transactions_whatsapp = document.querySelector("#transactions_whatsapp");
  ...
  if (document.querySelector("#transactions_mikrotik")) {
      var transactions_mikrotik = document.querySelector("#transactions_mikrotik");
      transactions_mikrotik.onsubmit = function (e) {
        e.preventDefault();
        loading.style.display = "flex";
        var request = window.XMLHttpRequest
          ? new XMLHttpRequest()
          : new ActiveXObject("Microsoft.XMLHTTP");
        var ajaxUrl = base_url + "/business/mikrotik";
        var formData = new FormData(transactions_mikrotik);
        request.open("POST", ajaxUrl, true);
        request.send(formData);
        request.onreadystatechange = function () {
          if (request.readyState == 4 && request.status == 200) {

...
function wspContactItem(client) {
  ...
  if (document.getElementById(`content-${client.id}`)) {
    return; // Si el cliente ya existe, no lo agregamos nuevamente
  }
  ...
  // Guardar datos en un input oculto
...
function getConcacts(state = 0, zona = "") {
  wspContactoClear();
  axios
    .get(`${base_url}/campania/list_users_record?state=${state}&zona=${zona}`)
    .then(({ data }) => {
...
function changeState() {
  ...
  const zona = document.getElementById("select-zona").value; // Nuevo selector de zonas
  if (!state && !zona) return wspContactoClear();
  getConcacts(state, zona);
}
...
function submitWsp() {
  document.getElementById("form-wsp").addEventListener("submit", async (e) => {
    ...
    execute().finally(() => {
      if (!errors.length) {
        wspClear();
        alert_msg("success", "Mensaje masivo enviado con éxito!!");
      } else if (errors.length && success.length) {
        alert_msg("warning", "Ocurrió un error al enviar algunos mensajes!!");
      } else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Inicializar eventos de cambio
  document.getElementById("select-state").addEventListener("change", changeState);
  document.getElementById("select-zona").addEventListener("change", changeZona); // Nuevo evento
});

...
class Bills extends Controllers
{
  ...
  public function list_records()
  {
    if ($_SESSION['permits_module']['v']) {
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        /* FACTURA DE SERVICIOS O PRODUCTOS / OBTENER MES DE LA FACTURA */
        if ($data[$i]['type'] == 1) {
          $month_letter = "";
      ...
      if ($idbill > 0) {
        ...
        if (!empty($data)) {
          if (empty($data['bill']['email'])) {
            ...
            $result = $email->send($_SESSION['businessData'], $data, $state);
            if (isset($result)) {
              return $this->json([
                "status" => "success",
  ...
  public function export()
  {
    ...
    $active_sheet->setAutoFilter('A1:Q1');

    //ejemplo de filtro a una celda
    // Obtener el rango de celdas para aplicar el filtro
    //$rango_celdas = 'C1:C' . $active_sheet->getHighestRow();
    // Agregar el filtro a la columna "C"
    //$active_sheet->setAutoFilter($rango_celdas);
    //$active_sheet = $spreadsheet->getActiveSheet();     
    // Obtener el rango de celdas para aplicar el filtro
    //$rango_celdas = 'P1:P' . $active_sheet->getHighestRow();
    // Agregar el filtro a la columna "C"
    //$active_sheet->setAutoFilter($rango_celdas);
    ...
    $spreadsheet->getActiveSheet()->getStyle('A1:Q1')->applyFromArray($style_header);
    $center_cell = array(
      'alignment' => array(
    ...
    $spreadsheet->getActiveSheet()->getStyle('C:M')->applyFromArray($center_cell);
    $spreadsheet->getActiveSheet()->getStyle('O:Q')->applyFromArray($center_cell);
    ...
    $active_sheet->setCellValue('C1', 'ZONA');
    ...
    $active_sheet->setCellValue('N1', 'PAGADO');
    $active_sheet->getColumnDimension('O')->setAutoSize(true);
    $active_sheet->setCellValue('O1', 'FORMA DE PAGO');
    $active_sheet->getColumnDimension('P')->setAutoSize(true);
    $active_sheet->setCellValue('P1', 'OBSERVACION');
    $active_sheet->getColumnDimension('Q')->setAutoSize(true);
    $active_sheet->setCellValue('Q1', 'ESTADO');
    ...
    if (!empty($data)) {
      ...
      foreach ($data as $key => $value) {
        ...
        $active_sheet->setCellValue('C' . $i, $value['zona']);
        $active_sheet->setCellValue('D' . $i, $value['mobile']);
        $active_sheet->setCellValue('E' . $i, $value['voucher'] . " Nº " . $invoice);
        $active_sheet->setCellValue('F' . $i, $month_letter);
        $active_sheet->setCellValue('G' . $i, $type_bill);
        $active_sheet->setCellValue('H' . $i, date("d/m/Y", strtotime($value['date_issue'])));
        $active_sheet->setCellValue('I' . $i, date("d/m/Y", strtotime($value['expiration_date'])));
        $active_sheet->setCellValue('J' . $i, empty($payments['payment_date']) ? "00/00/0000" : date("d/m/Y", strtotime($payments['payment_date'])));
        $active_sheet->setCellValue('K' . $i, format_money($value['subtotal']));
        $active_sheet->setCellValue('L' . $i, format_money($value['discount']));
        $active_sheet->setCellValue('M' . $i, format_money($value['total']));
        $active_sheet->setCellValue('N' . $i, empty($payments['amount']) ? format_money(0) : format_money($payments['amount']));
        $active_sheet->setCellValue('O' . $i, empty($payments['payment_type']) ? "" : $payments['payment_type']);
        $active_sheet->setCellValue('P' . $i, $value['observation']);
        $active_sheet->setCellValue('Q' . $i, $state);
        $i++;
      }
    }
  ...
  public function export_pendings()
  {
    ...
    $active_sheet = $spreadsheet->getActiveSheet();
    // Agregar filtro a la primera fila
    $active_sheet->setAutoFilter('A1:O1');
    $style_header = array(
      'font' => array(
        'name' => 'Calibri',
        'bold' => true,
    ...
    $spreadsheet->getActiveSheet()->getStyle('A')->applyFromArray($center_cell);
    $spreadsheet->getActiveSheet()->getStyle('C:K')->applyFromArray($center_cell);
    $spreadsheet->getActiveSheet()->getStyle('L:N')->applyFromArray($center_cell);
    ...
    $active_sheet->setCellValue('I1', 'F. VENCIMIENTO');
    $active_sheet->getColumnDimension('J')->setAutoSize(true);
    $active_sheet->setCellValue('J1', 'F. COMPROMISO');
    $active_sheet->getColumnDimension('K')->setAutoSize(true);
    $active_sheet->setCellValue('K1', 'SUBTOTAL');
    $active_sheet->getColumnDimension('L')->setAutoSize(true);
    $active_sheet->setCellValue('L1', 'DESCUENTO');
    ...
    if (!empty($data)) {
      ...
      foreach ($data as $key => $value) {
        ...
        if (isset($value['compromise_date'])) {
          $active_sheet->setCellValue('J' . $i, date_format(date_create($value['compromise_date']), "Y/m/d"));
        } else {
          $active_sheet->setCellValue('J' . $i, "");
        }
      ...
      if (empty($_POST['period'])) {
        ...
        if ($period_currect !== $period) {
          ...
          for ($i = 0; $i < count($data); $i++) {
            ...
            $request = $this->model->mass_registration($business, $user, $idclient, $voucher, $serie, $code, $correlative, $issue, $expiration, $current, $total, 0, $total, 2, 2, "", 2);
            if ($request > 0) {
              $idbill = $this->model->returnBill();
              $this->model->modify_available($voucher, $serie);
              $services = $this->model->select_detail_contract($data[$i]['id']);

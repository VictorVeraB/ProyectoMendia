...
class Customers extends Controllers
{
  ...
  {
    $orderDeuda = isset($_GET['orderDeuda']) ? $_GET['orderDeuda'] : null;
    if ($_SESSION['permits_module']['v']) {
      if (!empty($params)) {
      ...
      for ($i = 0; $i < count($data); $i++) {
        ...
        if (!empty($data[$i]['mobile_optional'])) {
          $mobile_optional_value = $data[$i]['mobile_optional'];
          $redirect_url = 'https://' . $mobile_optional_value;
          $mobiles_optional .= '<a href="' . $redirect_url . '" target="_blank"><i class="fa fa-street-view mr-1"></i>' . $mobile_optional_value . '</a>';
        }
        $data[$i]['ipcliente'] = $mobiles_optional;
        /* COORDENADAS */
  ...
  {
    if ($_POST) {
      if (empty($_POST['names']) || empty($_POST['surnames']) || empty($_POST['mobile']) || empty($_POST['address']) || empty($_POST['insDate'])) {
        ...
        $apCliente = strClean($_POST['ap_cliente_id']);
        if ($_POST['listPlan'] == 1) {
          $state = 1;
          $payday = intval($_POST['listPayday']);
          $invoice = isset($_POST['listInvoice']) ? intval($_POST['listInvoice']) : 0;
        ...
        if ($_SESSION['permits_module']['r']) {
          ...
          if (empty($existing_client)) {
            ...
            $payload_client = [
              ...
              "zona" => $zona,
              "mikrotik" => strClean($_POST['mikrotik']),
              "corte_firewall" => isset($_POST['corte_firewall']) ? 1 : 0,
              "email" => $email,
              "address" => $address,
              "reference" => $reference,
              "latitud" => "",
              "longitud" => "",
              "ap_cliente_id" => $apCliente,
              "nap_cliente_id" => strClean($_POST['nap_cliente_id']),
              "opcion" => strClean($_POST['opcion']),
              "ppoe_usuario" => strClean($_POST['ppoe_usuario']),
              "ppoe_password" => strClean($_POST['ppoe_password'])
            ];
            ...
            if ($request_client == 'success') {
              ...
              $mikrotik = strClean($payload_client['mikrotik']);
              $mikrotikOption = strClean($payload_client['opcion']);
              $mikrotikIp = strClean($payload_client['mobile_optional']);
              $mikrotikName = $idclient . " - " . $names . " " . $surnames;
              $mikrotikUser = $payload_client["ppoe_usuario"];
              $mikrotikPassword = $payload_client["ppoe_password"];

              // crear mikrotik
              if ($mikrotik && ($mikrotikOption == 'WISP' || $mikrotikIp)) {
                $apiMikrotik = new CustomerAssignSimpleQueue($mikrotik);
                $apiMikrotik->executeOnlyCreate($idclient, $service, $mikrotikName, $mikrotikIp);
              } else if ($mikrotik && $mikrotikOption == 'PPOE') {
                $apiMikrotik = new CustomerAssignPppSecret($mikrotik);
                $apiMikrotik->executeOnlyCreate($idclient, $service, $mikrotikName, $mikrotikUser, $mikrotikPassword);
              }
  ...
  public function modify_client()
  {
    try {
      if ($_POST) {
        if (empty($_POST['idclient']) || empty($_POST['names']) || empty($_POST['surnames']) || empty($_POST['mobile']) || empty($_POST['address'])) {
          $response = array("status" => 'error', "msg" => 'Campos señalados son obligatorios.');
        } else {
          $idclient = decrypt($_POST['idclient']);
          $idclient = intval($idclient);
          $information = contract_information($idclient);
          $client = $information['client'];
          if ($_SESSION['permits_module']['a']) {

            $mikrotikOption = strClean($_POST['opcion']);
            $corteFirewall = isset($_POST['corte_firewall']) ? 1 : 0;
            ...
            $payload = [
              ...
              "zona" => strClean($_POST['zona']),
              "mikrotik" => strClean($_POST['mikrotik']),
              "corte_firewall" => $corteFirewall,
              "email" => strtolower(strClean($_POST['email'])),
              "address" => strtoupper(clear_cadena(strClean($_POST['address']))),
              "reference" => strtoupper(strClean($_POST['reference'])),
              "latitud" => strClean($_POST['latitud']),
              "longitud" => strClean($_POST['longitud']),
              "ap_cliente_id" => $_POST['ap_cliente_id'],
              "nap_cliente_id" => $_POST['nap_cliente_id'],
              "opcion" => $mikrotikOption,
              "ppoe_usuario" => strClean($_POST['ppoe_usuario']),
              "ppoe_password" => strClean($_POST['ppoe_password'])
            ];

            $mikrotikChange = $information['client']['mikrotik'] != $payload['mikrotik'];
            $mikrotikName = $idclient . " - " . $payload['names'] . " " . $payload["surnames"];
            $mikrotikId = $payload['mikrotik'];
            $mikrotikIp = $payload['mobile_optional'];
            $mikrotikUser = $payload['ppoe_usuario'];
            $mikrotikPassword = $payload['ppoe_password'];
            $referen = $client['opcion'] != $mikrotikOption ? null : $client['mikrotik_referen'];

            // create info mikrotik
            if (($mikrotikChange || $client['opcion'] != $mikrotikOption) && $mikrotikId && ($mikrotikOption == 'WISP' || $mikrotikIp)) {
              $apiMikrotik = new CustomerAssignSimpleQueue($mikrotikId);
              $apiMikrotik->execute($idclient, $mikrotikName, $mikrotikIp);
            } else if (($mikrotikChange || $client['opcion'] != $mikrotikOption) && $mikrotikId && $mikrotikOption == 'PPOE') {
              $apiMikrotik = new CustomerAssignPppSecret($mikrotikId);
              $apiMikrotik->execute($idclient, $mikrotikName, $mikrotikUser, $mikrotikPassword);
            }

            // update info mikrotik
  ...
  public function layoff()
  {
    if ($_SESSION['permits_module']['e']) {
      if ($_POST) {
        $idcontract = decrypt($_POST['idcontract']);
        $idcontract = intval($idcontract);
        $client = $this->model->select_info_client_by_contract($idcontract);
        $service = new ClientSuspendService();
        $request = $service->execute($client['id']);
  ...
  public function activate()
  {
    if ($_SESSION['permits_module']['e']) {
      if ($_POST) {
        ...
        $request = $service->execute($idcontract);
  ...
  public function export()
  {
    ...
    // Agregar filtro a la primera fila
    $active_sheet->setAutoFilter('A1:R1');
    ...
    $spreadsheet->getActiveSheet()->getStyle('A1:R1')->applyFromArray($style_header);
    $center_cell = array(
      'alignment' => array(
    ...
    $spreadsheet->getActiveSheet()->getStyle('F')->applyFromArray($center_cell);
    ...
    $spreadsheet->getActiveSheet()->getStyle('L:R')->applyFromArray($center_cell);
    $active_sheet = $spreadsheet->getActiveSheet();
    ...
    if (!empty($data)) {
      ...
      foreach ($data as $key => $value) {
        ...
        $active_sheet->setCellValue('F' . $i, $value['mobile_optional']);
        $active_sheet->setCellValue('G' . $i, $value['zona']);
        $active_sheet->setCellValue('H' . $i, $value['email']);
        $active_sheet->setCellValue('I' . $i, $value['address']);
        $active_sheet->setCellValue('J' . $i, $value['reference']);
        $active_sheet->setCellValue('K' . $i, $value['latitud']);
        $active_sheet->setCellValue('L' . $i, $value['longitud']);
        $active_sheet->setCellValue('M' . $i, $serv);
        $active_sheet->setCellValue('N' . $i, empty($lastpayment) ? "00/00/0000" : date("d/m/Y", strtotime($lastpayment)));
        $active_sheet->setCellValue('O' . $i, ($value['state'] == 5) ? 0 : $payday);
        $active_sheet->setCellValue('P' . $i, $pending_payments);
        $active_sheet->setCellValue('Q' . $i, $payment_date);
        $active_sheet->setCellValue('R' . $i, $state);
        $i++;
      }
    }
              ...
              if ($type == 1) {
                if ($request == "success") {
                  ...
                  if ($method == 1) {
                    ...
                    if ($request == "success") {
                      $this->model->modify_amounts($idbill, $total_bill, $total_bill, 0, 1);
                    }
                  }
  ...
  public function send_email(string $params)
  {
    ...
    try {
      ...
      if ($idbill > 0) {
        ...
        if (!empty($data)) {
          if (empty($data['bill']['email'])) {
            ...
            $result = $email->send($_SESSION['businessData'], $data, $affair);
            if (isset($result)) {
              return $this->json([
                "status" => "success",
              ...
              return $this->json([
                "status" => "error",
                "msg" => "Hubo un error,reenvié el correo nuevamente."
              ]);
            }
          }
        }
      ...
      return $this->json([
        "status" => "error",
        "msg" => "La información buscada no existe."
      ]);
    } catch (\Throwable $th) {
      $this->json([
        "success" => "error",
  ...
  {
    if ($_POST) {
      if (empty($_POST['idclient']) || empty($_POST['idbill']) || empty($_POST['total_pay'])) {
        $response = array("status" => 'error', "msg" => 'Campos señalados son obligatorios');
      } else {
        ...
        $subscriber = strClean($_POST['total_pay']);
        if ($id == 0) {
          $option = 1;
          if ($_SESSION['permits_module']['r']) {
            ...
            $remaining = $this->model->remaining_amount_data($bill);
            $discount = (float) $_POST['total_discount'];
            $subtotal = (float) $remaining['subtotal'];
            $amount_paid = (float) $remaining['amount_paid'];
            ...
            if ($remaining_amount == 0) {
              ...
              if ($subtract_amounts == 0) {
                $this->model->modify_amounts($bill, $subscriber, 0, 1, $discount, $date_compromiso);
                ...
                $this->model->modify_amounts($bill, $subscriber, $subtract_amounts, 0, $discount, $date_compromiso);
                $amount_paid = $this->model->amount_paid($bill);
  ...
  public function search_service()
  {
    ...
    if (!$search)
      return $this->json([]);
    $data = $this->model->search_service($search);
    $data = array_map(function ($item) {
      $infoType = $item['type'] == 1 ? "- INTERNET DE BAJADA: {$item['descent']} - SUBIDA: {$item['rise']}" : "";
      $symbol = $_SESSION['businessData']['symbol'];
      $price = format_money($item['price']);
      $item['idEncrypt'] = encrypt($item['id']);
      $item['infoService'] = "{$item['service']} - {$symbol} {$price} {$infoType}";
      return $item;
    }, $data);
  ...
  public function action_service()
  {
    if ($_POST) {
      if (empty($_POST['price']) || empty($_POST['idcontract']) || empty($_POST['listService'])) {
        ...
        $service = new ClientUpdateMikrotikService();
        ...
        if ($id == 0) {
          ...
          if ($_SESSION['permits_module']['r']) {
            ...
            $service->executeByContractId($idcontract);
          }
        } else {
          $option = 2;
          if ($_SESSION['permits_module']['a']) {
  ...
  public function update_router(string $id)
  {
    try {
      $this->model->editClient($id, [
        "route_ssid" => $_POST["route_ssid"],
        "route_password" => $_POST["route_password"],
        "route_cred_user" => $_POST["route_cred_user"],
        "route_cred_password" => $_POST["route_cred_password"]
      ]);
      $this->json([
        "success" => true,
      ...
      $this->json([
        "success" => false,
        "message" => "No se pudo actulizar los datos",
        "error" => $th->getMessage()
      ]);
    }
  }

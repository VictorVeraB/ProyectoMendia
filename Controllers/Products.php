<?php
    ...
    class Products extends Controllers{
        ...
        public function export(){
            ...
            $active_sheet->setCellValue('J1', 'PRESENTACIÓN');
            $active_sheet->getColumnDimension('K')->setAutoSize(true);
            $active_sheet->setCellValue('K1', 'PROVEEDOR');
            $active_sheet->getColumnDimension('L')->setAutoSize(true);
            $active_sheet->setCellValue('L1', 'DESCRIPCIÓN');
            ...
            if (!empty($data)){
                ...
                foreach ($data as $key => $value) {
                    ...
                    $active_sheet->setCellValue('J'.$i, $value['united']);
                    $active_sheet->setCellValue('K'.$i, $value['provider']);
                    $active_sheet->setCellValue('L'.$i, $value['description']);
                    $i++;
                }
            }
